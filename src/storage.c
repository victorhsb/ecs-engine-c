...
#include <stdlib.h>
//
// Position Storage serves as the reference point for all the other storage
// implementations that may or may not be written using macros just for the sake
// of practicity.
//
...
PositionStorage PositionStorage_init(void) { return (PositionStorage){0}; }

void PositionStorage_destroy(PositionStorage *storage) { (void)storage; }
...
bool has_position(PositionStorage const *storage, Entity const entity) {
    uint32_t idx = get_entity_index(entity),
             ...
             dense_gen = dense_index > 0
                             ? get_entity_generation(
                                   storage->dense_entities[dense_index])
                             : 0;
    return dense_index > 0 && dense_gen == gen;
}

void upsert_position(PositionStorage *storage, Entity entity,
                     Position position) {
    // optimistic component upsert. does not take into consideration if entity
    // is alive. if the position for that entity exists already we just set it.
    uint32_t entity_idx = get_entity_index(entity),
             dense_idx = storage->sparse_entities[entity_idx];
    if (dense_idx > 0) {
        uint32_t stored_gen =
                     get_entity_generation(storage->dense_entities[dense_idx]),
    ...
    // 4. finally update the sparse entity array with the dense index on the
    // entity index.
...
void remove_position(PositionStorage *storage, Entity entity) {
    ...
    if (dense_index != last_index) {
        ...
        storage->dense_entities[dense_index] =
            storage->dense_entities[last_index];
        storage->sparse_entities[get_entity_index(
            storage->dense_entities[dense_index])] = dense_index;
    }
...
// -------------------------------------------------------------------------------------------------------
// Other storages might not need that much of a deep implementation. Not like
// the singleton storages such as ball and paddle though.
// -------------------------------------------------------------------------------------------------------

#define GENERATE_STORAGE_INIT(tgt)                                             \
    void tgt##_destroy(tgt *storage) { (void)storage; }                        \
    tgt tgt##_init(void) { return (tgt){0}; }
...
#define GENERATE_STORAGE_CRUD(COMP, PREFIX)                                    \
    bool has_##PREFIX(COMP##Storage *storage, Entity entity) {                 \
        ...
        uint32_t dense_gen =                                                   \
            dense_index > 0                                                    \
                ? get_entity_generation(storage->dense_entities[dense_index])  \
                : 0;                                                           \
        return dense_index > 0 && dense_gen == gen;                            \
    }                                                                          \
                                                                               \
    void upsert_##PREFIX(COMP##Storage *storage, Entity entity, COMP value) {  \
        ...
        if (dense_idx > 0) {                                                   \
            uint32_t stored_gen = get_entity_generation(                       \
                         storage->dense_entities[dense_idx]),                  \
                     incoming_gen = get_entity_generation(entity);             \
            if (stored_gen == incoming_gen) {                                  \
    ...
    void remove_##PREFIX(COMP##Storage *storage, Entity entity) {              \
        ...
        storage->sparse_entities[entity_index] = 0;                            \
        if (dense_index != last_index) {                                       \
            storage->dense_data[dense_index] =                                 \
                storage->dense_data[last_index];                               \
            storage->dense_entities[dense_index] =                             \
                storage->dense_entities[last_index];                           \
            storage->sparse_entities[get_entity_index(                         \
                storage->dense_entities[dense_index])] = dense_index;          \
    ...
    COMP *get_##PREFIX(COMP##Storage *storage, Entity entity) {                \
        if (!has_##PREFIX(storage, entity))                                    \
            return NULL;                                                       \
        uint32_t dense_idx =                                                   \
            storage->sparse_entities[get_entity_index(entity)];                \
        return &storage->dense_data[dense_idx];                                \
    }
